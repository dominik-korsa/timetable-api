import { School, SchoolSpecifier, slugify, SpecifierMinimum, isDefined } from '@timetable-api/common';
...
function createSpecifier(
    ...
    if (minimum === 'county' && territoriesConfig.duplicateCounties.has(rspoResponse.powiatKodTERYT))
        minimum = 'voivodeship';
    ...
    const minParts = (
        {
            ...
            county: 4,
            commune: 3,
        } satisfies Record<SpecifierMinimum, number>
    )[specifier.minimum];
...
export async function loadSchools(): Promise<School[]> {
    ...
    return (
        await Promise.all(
            schoolConfigs.map(async (config) => {
...
export async function updateSchoolCache() {
    ...
    schools.forEach((school) => {
        transaction.hSet('schools', school.rspoId, JSON.stringify(school));
