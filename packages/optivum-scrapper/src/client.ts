...
export async function parse(url: string) {
    ...
    const tableWithMaxTimeSlotx = units.reduce((a, b) =>
        a.table.getTimeSlotCount() > b.table.getTimeSlotCount() ? a : b,
    );
    const tableWithMaxTimeSlots = tableWithMaxTimeSlotx.table;
    ...
    await Promise.all(
        units.map((unit) => {
            unit.table.getLessons().map((lesson) => {
